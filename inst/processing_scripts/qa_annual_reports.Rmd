...
```{r animal_diseases7}
## Cases where there are mismatches between semester and annual reports - due to being listed as more than one status (above)

# animal_diseases_check %>%
#   group_by(country_iso3c, report_year, report_semester, disease, oie_listed) %>%
#   mutate(n = n()) %>%
#   ungroup() %>%
#   filter(n>2) %>%
#   arrange(n, country_iso3c, report_year, report_semester, disease) %>%
#   make_dt()

```

## Cases where the new outbreaks in the annual report do not match the sum of the new outbreak in the semester reports (some due to name mismatches or missing/error reports (see one_or_more_report_missing))
```{r animal_diseases8}
...
animal_diseases_check %>%
  ...
  group_by(country_iso3c, report_year, report_semester, disease, oie_listed, disease_population.sem, disease_population.ann) %>%
  mutate(n = n()) %>%
  ungroup() %>%
  arrange(n, country_iso3c, report_year, report_semester, disease) %>%
  filter(n<=2) %>%
  group_by(country_iso3c, report_year, disease, oie_listed, disease_population.sem, disease_population.ann) %>%
  ...
  select(-match_new_outbreaks) %>%
  left_join(status_missing %>%
              select(-report_semester, -status) %>%
              distinct() %>%
              mutate(one_or_more_report_missing = TRUE)) %>%
  mutate(one_or_more_report_missing = replace_na(one_or_more_report_missing, FALSE)) %>%
  make_dt()
```
